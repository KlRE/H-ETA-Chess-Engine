...
uint64_t Board::sliderBlockers(Square sq, Color color)  // inspiration from stockfish:position.cpp
{
  ...
  uint64_t snipers = ((getP(!color, Rook)|getP(!color, Queen)) & mRook(sq, 0ull)) | ((getP(!color, Bishop) | getP(!color, Queen)) & mBishop(sq, 0ull));
  //drawB(snipers);
...
void Board::setArr(char Arr[64], Color color, string castling, Square epSq) // when calling this function add color as parameter, otherwise calls wrong one
{
  SideToMove = color;
...
void Board::drawBoard()
{
  ...
  cout << "Castling Rights: " << (castle[Queenside][SideToMove]?"Queenside ":"") << (castle[Kingside][SideToMove]?"Kingside":"") << "\n";
