...
Move* Board::generatePawnM(Move* list, const uint64_t &pinned, const uint64_t &pushMask, const uint64_t &captureMask, uint64_t pos, int kingSq)
{
	  ...
	  if(((epSqBb & pushMask & pinnedLine)    //if the pawn ends up on a moveable square
				     || (capSqBb & captureMask & pinnedLine)))  // if pawn captures capturable square
	    {
	      ...
	      uint64_t kingCheck = (epLine & mRook(kingSq, occwoPawn) & (getP(!SideToMove,Rook) | getP(!SideToMove, Queen)))
		                 | (getLines(kingSq, lsb(capSqBb)) & mBishop(kingSq, occwoPawn) & (getP(!SideToMove, Bishop) | getP(!SideToMove, Queen)));
	      //drawB(kingCheck);
...
Move* Board::generateRookM(Move* list, const uint64_t &pinned, const uint64_t &pushMask, const uint64_t &captureMask, uint64_t pos, int kingSq)
{
  while(pos)
    {
      Square from = pop_lsb(pos);
...
Move* Board::generateBishopM(Move* list, const uint64_t &pinned, const uint64_t &pushMask, const uint64_t &captureMask, uint64_t pos, int kingSq)
{
  while(pos)
    {
      Square from = pop_lsb(pos);
...
Move* Board::generateKnightM(Move* list, const uint64_t &pinned, const uint64_t &pushMask, const uint64_t &captureMask, uint64_t pos)
{
  while(pos)
    {
      Square from = pop_lsb(pos);
...
Move* Board::generateQueenM(Move* list, const uint64_t &pinned, const uint64_t &pushMask, const uint64_t &captureMask, uint64_t pos, int kingSq)
{
...
Move* Board::generateMoves(Move *list)
{
  ...
  vector<Square> OpPawn = PieceSqs(!SideToMove, Pawn);   //maybe remove the conversion to vector and doing it with while(bitboard) pop_lsb(bitboard) (s. surge-master)
  ...
  uint64_t checkers = attacksTo(OuKing, !SideToMove); //maybe no need to check for queen and rook bc it can be done later with pinning step (s. surge-master)
  //drawB(checkers);
